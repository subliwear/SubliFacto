...
const HomePage5Submit = (values, mutate) => {
    ...
    if (values['productDealsImage']) {
        values['content']['product_with_deals']['deal_of_days']['image_url'] = values['productDealsImage'].asset_url
    ...
    values['content']['bank_wallet_offers']['offers'].forEach((elem, i) => {
        if (values[`bankOfferImage${i}`]) {
            values['content']['bank_wallet_offers']['offers'][i]['image_url'] = values[`bankOfferImage${i}`].asset_url
        } else values['content']['bank_wallet_offers']['offers'][i]['image_url'] = ""
    })

    values['content']['bank_wallet_offers']['offers'].forEach((elem, i) => {
        if (values[`bankOfferBankImage${i}`]) {
            values['content']['bank_wallet_offers']['offers'][i]['bank_image_url'] = values[`bankOfferBankImage${i}`].asset_url
