const HomePage5InitialValue = (data) => {
    ...
    data?.content?.bank_wallet_offers?.offers.length > 0 &&
        data?.content?.bank_wallet_offers?.offers.forEach((elem, index) => {
            elem.image_url ? obj[`bankOfferImage${index}`] = { asset_url: elem.image_url } : ''
            return obj
        })
    let obj2 = {}
    data?.content?.bank_wallet_offers?.offers.length > 0 &&
        data?.content?.bank_wallet_offers?.offers.forEach((elem, index) => {
            elem.bank_image_url ? obj2[`bankOfferBankImage${index}`] = { asset_url: elem.bank_image_url } : ''
            return obj2
        })
    ...
    data?.content?.featured_banners?.banners.length > 0 &&
        data?.content?.featured_banners?.banners.forEach((elem, index) => {
            elem.image_url ? obj4[`featureBannerImage${index}`] = { asset_url: elem.image_url } : ''
    ...
    return {
        ...
        // Images
        productDealsImage: data?.content?.product_with_deals?.deal_of_days?.image_url ? { asset_url: data?.content?.product_with_deals?.deal_of_days?.image_url } : "",
